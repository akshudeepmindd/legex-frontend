	...
	return async (dispatch) => {
		...
		try {
			...
			if (response.data.data.ownerType === "Organization") {
				dispatch({ type: DELETE_ORGANIZATION_DOCUMENT_SUCCESS, payload: response.data.data })
			} else if (response.data.data.ownerType === "User") {
				dispatch({ type: DELETE_USER_DOCUMENT_SUCCESS, payload: response.data.data })
			}
			return true
		} catch (error) {
			return false
	...
	return async (dispatch) => {
		...
		try {
			...
			if (response.data.data[0].ownerType === "Organization") {
				dispatch({ type: UPLOAD_ORGANIZATION_DOCUMENT_SUCCESS, payload: response.data.data })
			} else if (response.data.data[0].ownerType === "User") {
				dispatch({ type: UPLOAD_USER_DOCUMENT_SUCCESS, payload: response.data.data })
			}
			return true
		} catch (error) {
			return false
